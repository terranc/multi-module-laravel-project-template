...
(function(){
    ...
    function insertSingle(){
        ...
        if(/^<iframe/.test(url)){
            ...
            var div;
            if (url.match(/(?<=width=\")\d+(?=\")/g)) {
                width.value = url.match(/(?<=width=\")\d+(?=\")/g);
            } else {
                width.value = 510;
            }
            console.log(width.value);
            if (url.match(/(?<=height=\")\d+(?=\")/g)) {
                height.value = url.match(/(?<=height=\")\d+(?=\")/g);
            } else {
                height.value = 498;
            }
    ...
    function convert_url(url){
        ...
        url = utils.trim(url)
            ...
            .replace(/v\.qq\.com\/x\/cover\/[\w\/]+\/([\w]+)\.html/i, "v.qq.com/txp/iframe/player.html?vid=$1")
            .replace(/v\.qq\.com\/x\/.+[\?\&]vid=([^&]+).*$/i, "v.qq.com/txp/iframe/player.html?vid=$1")
