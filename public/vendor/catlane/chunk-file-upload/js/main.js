function chunk_file ( name , accept , disk , driver ) {
    ...
    var options = {
        ...
        nameUrl: window.chunk_file.prefix + '/' + "chunk-file-upload/get_name" ,
        mockToken: false ,
        hash: false ,
        disk: disk,
        namePrefix : ''
    ...
    uploader.on ( "error" , function ( type , handler ) {
        switch ( type ) {
            ...
            case 'Q_EXCEED_SIZE_LIMIT':
                swal ( '上传文件大小过大' , '' , 'error' ).then ( function () {
                } );
                break;
    ...
    function GetName ( nameUrl  ) {
       $.ajax ( {
            ...
            success: function ( res ) {
                options.namePrefix = res;
            }
        } );
    }

    uploader.on ( "uploadStart" , function ( file ) {
        GetName(options.nameUrl);
        var name = options.namePrefix + Date.parse(new Date()) + Math.random().toString(36).substr(2) + '.' + file.source.ext;

        if ( driver == 'qiniu' ) {//七牛，传递七牛key
            if ( !options.mockToken ) {
                GetToken(options.tokenUrl, file, name);
            } else {
    ...
    function GetToken(tokenUrl, file, name) {
        $.ajax ( {
            async: false ,
            type: 'get' ,
            url: tokenUrl ,
    ...
    $ ( document ).on ( 'click' , '#' + name + '-savedpath' , function () {
        window.open ( window.chunk_file.url + '/' + $ ( this ).val () );
    } );
    return uploader;
